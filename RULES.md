...
These extend the foundry root rules (`/home/ivaldi/foundry/RULES.md`). Where they
overlap, the root rules win.
...
2. **Agent-authored commits.** Every commit passes identity explicitly — the author name read
   at runtime from the env (the model you're running as), never a hardcoded version:
   `git -c user.name="$FOUNDRY_COMMIT_AUTHOR_NAME" -c user.email="${FOUNDRY_COMMIT_AUTHOR_EMAIL:-noreply@anthropic.com}" commit ...`
...
5. **Browser-tested or it isn't done.** UI changes are verified with Playwright
   ...
   `console.error`), per foundry root RULES §7. Headless WebGL2 works here
